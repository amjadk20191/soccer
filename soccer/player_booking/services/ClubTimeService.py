...
class ClubTimeService:
    ...
    @classmethod
    def get_opening_time_with_pitches_prices(cls, club_id, number_of_day):
        """
    ...
    def get_club_general_time(cls, club_id):
        ...
        """
        club = Club.objects.values('open_time', 'close_time').filter(id=club_id).first()
        if not club:
    ...
    def get_opening_time_with_percent(cls, club_id, number_of_day):
        ...
        open_time, close_time = cls.get_club_general_time(club_id)
